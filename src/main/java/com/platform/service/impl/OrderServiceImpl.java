...
public class OrderServiceImpl  implements OrderService{
	...
	public void addorder(Order order) {
		...
		User user = userMapper.finduserById(order.getUser_id());      // 根据   用户的ID 查看 该 用户信息
		GoodsForWeb goods = goodsMapper.findGoodsinfoByGoodsId(order.getGoods_id()) ;   // 根据 商品的ID查 该商品
System.out.println(" 商品价格 ：" + goods.getGoods_price() + "  goodsId :" + order.getGoods_id());			
		if(user.getUser_type().equals("4")){           // 订单是  会员的订单
			...
			if(goods.getGoods_return_type() == 1 ){        // 根据金额返券
				if(goods.getGoods_return_standard()< order.getGooods_number() * (goods.getGoods_price()*1.00/100) ){
					order.setReturn_number(goods.getGoods_return_ticket());
				}
				else{
					order.setReturn_number(0);
...
System.out.println("扣除logn币  的个数： "  + order.getLB_money());	
		...
		String url = "http://124.254.56.58:8007/api/Member/MemberLongBiPayment?money="
				+ order.getLB_money()+"&recipientname="
				+ userLogin+"&pw="
				+ payPass  ;
...
System.out.println("进入点子币支付.........支付密码 ：" + payPass);
	...
	System.out.println("扣除电子币  的个数： "  + order.getElectronics_money() / 100);	
		...
		String url = "http://124.254.56.58:8007/api/Member/MemberElectronicMoneyPayment?money="
				+ order.getElectronics_money()/100+"&recipientname="
				+ userLogin+"&pw="
				+ payPass  ; 
		...
		System.out.println("电子币支付成功");
		          ...
		          Order dOrder = orderMapper.selectallpay_state(order.getOrder_id()) ;
		          if(dOrder.getDianzibi_pay_state() == 1){   // 第二次支付时 ，不修改订单 中电子币 的数量 ；
	...
	public Pay_info  update_Sendorder_yinlian(Json_send send ,Integer pay_type) {
		...
		order.setEffectiveTime(send.getEffectiveTime());// 订单有效期期限（秒），值小于等于 0 表示订单长期有效
		
		order.setOrderDate(send.getOrderDate());// 订单日期     curreTime.substring(0, 8)
