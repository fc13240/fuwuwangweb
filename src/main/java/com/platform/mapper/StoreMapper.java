...
public interface StoreMapper {
	...
	//安卓端
	public List<StoreForApp>   selectstore(Store_help  sHelp);    // 根据 street_id 获得所有的对应的store ；
	
	//安卓端
	public List<StoreForApp>   selectstoreByname(String  likename);  // 模糊的name 查所有店铺 ；
	
	//安卓端
	public List<StoreForApp>   selectstoreByNameAndCity_id(@Param(value="likename")String  likename,@Param(value="city_id")Integer city_id);  // 模糊的name 查所有店铺 ；
	
	//安卓端
	public List<StoreForApp>   findStoreByType_Street(Store_help  sh);  // 根据  type1 和 street_id查店铺；
	
	//安卓端
	public List<Store_type1>  selectClass_1();                 // 查 表t_store_type1中的 六个 类
	
	//安卓端
	public List<Store_type2>  selectClass_2(Integer  store_type1_id);        // 根据type1查 type2
	
	//安卓端
    public List<StoreForApp>  selectStoreBytype2(Integer  store_type2_id);        // 根据type2查 店铺
	
    //安卓端
    public List<StoreForApp>  selectStoreBytype2_street(Store_help sHelp);        // 根据type2 and street查 店铺
    
    public StoreForApp findStoreinfoByStore_idForApp(String store_id);  //  根据 店铺id 查店铺
	//web端
	public void updateStoreState(Store_state  Ss);                  // 根据店铺的 id 修改店铺的状态，锁定，解锁，审核 ，不审核
	...
	public Page<StoreForWeb>  findstoreByNameAndCity_id(@Param(value="store_name")String store_name,@Param(value="city_id")Integer city_id);  // 根据名字模糊查找所有的店铺
	...
	public  List<StoreForWeb> findStoreOrderByStatus() ;   // 查询店铺根据状态排序
		...
		public List<StoreForApp>   selectStoreByNameAndCityName(@Param(value="searchName")String searchName,
				@Param(value="city_name")String cityName,@Param(value="pageSize") Integer pageSize,@Param(value="offset") Integer offset);  
		...
		public List<StoreForApp>  selectStoreByConditions(Store_help  sHelp); //根据城市  街道   一级分类 二级分类 获取 正常状态下的 “店铺” 信息
		
		
		public Integer  selectStoreCountByConditions(Store_help  sHelp); //根据城市  街道   一级分类 二级分类 获取 正常状态下的 “店铺总数”
