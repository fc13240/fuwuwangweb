...
function check(){ /* passWord newpass */
	...
	{ 
	$("#newpassLabel").text("新密码不能为空！") 
	$("#newpassLabel").css({"color":"red"}); 
	isSuccess = 0; 
	} 
	 
	if(txt_newpass.length <6||txt_newpass.length > 8) 
	{ 
	$("#newpassLabel").text("密码应为6-8位！") 
	$("#newpassLabel").css({"color":"red"}); 
	isSuccess = 0; 
	} 
	if(txt_newpass2.length == 0) 
	{ 
	$("#newpassLabel2").text("密码不能为空！") 
	$("#newpassLabel2").css({"color":"red"}); 
	isSuccess = 0; 
	} 
	
	if(txt_newpass2.length < 6||txt_newpass2.length > 8) 
	{ 
...
<div class="container-fluid">
	...
	<form class="form-horizontal"
		action="${pageContext.request.contextPath}/admin/user/userinfo"
		...
		<div class="form-group">
			...
			<div class="col-sm-4">
				<input type="password" name="passWord" class="form-control" id="passWord"
					placeholder="请输入旧密码" onpaste="return false" ondragenter="return false" oncontextmenu="return false;" style="ime-mode:disabled" ><label  id="passWordLabel"></label>
			</div>
		</div>
		...
		<div class="form-group">
			...
			<div class="col-sm-4">
				<input type="password" class="form-control" name="newpass" id="newpass"
					placeholder="请输入新密码" onpaste="return false" ondragenter="return false" oncontextmenu="return false;" style="ime-mode:disabled"><label  id="newpassLabel"></label>
			</div>
		</div>
		<div class="form-group">
