...
public class AdminFinancialController {
	...
	public String goodsfinancial(String goodsname, String order_time_start, String order_time_end, Model model,
			String type) {

		List<Order> listFINan = new ArrayList<Order>();
		System.out.println("商品销售信息查询" + type);
		if (type.equals("0")) {
			listFINan = financialService.goodsfinancial("", DateUtil.getyyMM() + "-01", DateUtil.getyyMM() + "-31");
			System.out.println("当月 ：起始时间-" + DateUtil.getDay() + " 终止时间 ：" + DateUtil.getyyMM() + "-31");
		} else {
			if (order_time_start.length() > 0 && order_time_end.length() == 0) {
				order_time_end = DateUtil.getDay();
			} else if (order_time_start.length() == 0 && order_time_end.length() == 0) {
